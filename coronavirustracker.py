import requests      #for getting a data from online server or sites
from bs4 import BeautifulSoup
import plyer


def datacollected(): #we uses this function to make it run easily
    def notification(title, message):
        plyer.notification.notify( 
        title= title,
        message= message,
        app_icon = 'corona.ico', #for app icon during notification occurs
        timeout = 15      # we will keep notification for 15 seconds
        )

    url = "https://www.worldometers.info/coronavirus/" #its a urs of worldometer
    res = requests.get(url)                            #getting data from url
    soup = BeautifulSoup(res.content,'html.parser') 
    tbody = soup.find('tbody')    #here tbody is total body in inspect element of worldometer
    abc = tbody.find_all('tr')    #tr is table row
    countrynotification = cntdata.get() 
    if(countrynotification == ""):
        countrynotification = "world" # we will keep world as by default when no country is entered
    ...
    ## header are used to name the column in file 
...
#from here Buttons code will starts-------------------->>>>>>>>>>>>>>
